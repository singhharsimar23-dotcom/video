    ...
    <div className="controls">
      <div className="field">
        ...
        <div className="toggle-group">
          <button className={`toggle-btn ${value.format === 'reels' ? 'active' : ''}`} onClick={() => update('format', 'reels' as VideoFormat)}>REELS 9:16</button>
          <button className={`toggle-btn ${value.format === 'longform' ? 'active' : ''}`} onClick={() => update('format', 'longform' as VideoFormat)}>LONG-FORM 16:9</button>
        </div>
      </div>
      <div className="field">
        ...
        <button className="btn-secondary" style={{ marginTop: 8 }} onClick={() => update('topic', '')}>Surprise me</button>
      </div>
      <div className="field">
      ...
      <button className="btn-primary" disabled={busy} onClick={onSubmit}>{busy ? 'Generating…' : 'Generate Video'}</button>
    </div>
  );
}
